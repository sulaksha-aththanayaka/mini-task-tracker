...
const PriorityCard = ({ data }: PriorityStatCardProps) => {
  return (
    <Card className={cn("w-full max-w-sm shadow-sm border-l-4 md:border-l-16 flex flex-col gap-0 justify-center py-2", data.borderColor)}>
      <CardHeader className="px-2 sm:px-4 lg:px-6 md:pb-2 h-8">
        <CardTitle className="text-sm md:text-lg lg:text-xl font-bold uppercase text-muted-foreground tracking-wider">{data.label}</CardTitle>
      </CardHeader>
